...
public class Calculator {
    ...
    //private Stream<Dice> piercerStream;
    ...
    public double calculateDprAgainstAC(PlayerCharacter pc, int theAc) {
        ...
        vexHeap = new double[(int)Math.pow(2, pc.getNum_of_attacks() + pc.getNum_of_attacks_bonus()) - 1];
        if (pc.isGwm_check()) {
    ...
    public double calculateDamageAgainstAC(boolean hasAdv, boolean hasElAc, int critRange, int theAc, double aMod, double damagePerStandardHit, double standardMod, double damagePerStandardHitCrit,
        ...
        //SAdamage and DPFH could be combined to reduce redundancy
        if (!isGwm) {
        ...
        if (hasPiercer) {
            ...
            piercerDamage += (1 - chanceOfMissPiercer) * calculatePiercer(piercerDie, piercerRerollNum, piercerParamGwf);
        }
        totalDamage += piercerDamage;
     ...
     */
    //Vex needs tuning; Issues with interactions with GWM, Crusher, Stalker's Flurry
